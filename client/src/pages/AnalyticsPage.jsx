  ...
  if (active && payload?.length) {
    return (
      <div className="glass rounded-lg p-3 text-sm border border-white/10">
        ...
        {payload.map((p, i) => (
          <p key={i} className="text-nexura-text-dim">
            {p.name}:{" "}
...
export default function AnalyticsPage() {
  ...
  const workloadData =
    analytics?.facultyWorkload?.map((f) => ({
  ...
  const deptData = analytics?.departmentStats
    ? Object.entries(analytics.departmentStats).map(([dept, d]) => ({
        name: dept,
        students: d.students,
        faculty: d.faculty,
        courses: d.courses,
  ...
  const avgUtilization =
    workloadData.length > 0
      ? Math.round(
          workloadData.reduce((s, w) => s + w.utilization, 0) /
            workloadData.length,
        )
      : 0;
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-nexura-text-dim text-sm sm:text-base">
              Faculty workload, room utilization, and conflict analysis
            </p>
          </div>
            ...
            {[
              ...
              <GlassCard key={i} delay={i * 0.1}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-nexura-text-muted leading-tight">
                    {item.label}
                  </span>
                  <item.icon className={`w-4 h-4 shrink-0 ${item.color}`} />
                </div>
                <div
                  className={`text-xl sm:text-2xl font-bold font-display ${item.color}`}
                >
                  {item.value}
                </div>
              </GlassCard>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            <GlassCard hover={false}>
              <h3 className="text-base sm:text-lg font-semibold text-nexura-text mb-4 flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-nexura-cyan" /> Faculty
                Workload
                <span className="text-xs text-nexura-text-muted font-normal">
                  (hrs/week)
                </span>
              </h3>
              {/* chart height smaller on mobile so it doesn't overflow */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <GlassCard hover={false}>
              <h3 className="text-base sm:text-lg font-semibold text-nexura-text mb-4 flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-pink-400" /> Department
                Comparison
              </h3>
              <div className="h-56 sm:h-72">
                <ResponsiveContainer width="100%" height="100%">
            ...
            <GlassCard hover={false}>
              <h3 className="text-base sm:text-lg font-semibold text-nexura-text mb-4 flex items-center gap-2">
                <Activity className="w-5 h-5 text-emerald-400" /> Conflict
                Resolution Rate
              </h3>
              <div className="h-56 sm:h-72 flex flex-col items-center justify-center">
                ...
                <div className="text-center -mt-10 sm:-mt-16">
                  <div className="text-3xl sm:text-4xl font-bold font-display text-emerald-400">
                    97%
                  </div>
                  <div className="text-xs sm:text-sm text-nexura-text-dim mt-1">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 h-2 bg-white/5 rounded-full overflow-hidden">
                      <motion.div
                        ...
                        className={`h-full rounded-full ${f.utilization > 80 ? "bg-red-500" : f.utilization > 60 ? "bg-amber-500" : "bg-emerald-500"}`}
                      />
                    </div>
                    <span className="text-xs text-nexura-text-dim shrink-0">
                      {f.utilization}%
                    </span>
                  </div>
                  <div className="text-xs text-nexura-text-muted mt-1">
                    {f.hoursPerWeek}h / {f.maxHours}h per week
                  </div>
                </motion.div>
              ))}
            </div>
                ...
                <tbody>
                  {analytics?.facultyWorkload?.map((f, i) => (
                    ...
                    >
                      ...
                      <td className="py-3 pr-4 text-nexura-text-dim">
                        {f.maxHours}
                      </td>
                      <td className="py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-nexura-text-dim">
                            {f.utilization}%
                          </span>
                        </div>
                      </td>
