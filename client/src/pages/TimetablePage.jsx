...
export default function TimetablePage() {
  ...
  useEffect(() => {
    ...
    timetableAPI
      .getCurrent()
      .then((r) => {
        if (r.data.success && r.data.data) setTimetable(r.data.data);
      })
      .catch(() => {});
  }, []);
  ...
  const handleSimulate = async () => {
    ...
    setSimulating(true);
    try {
      const res = await timetableAPI.simulateChange(unavailable);
      setTimetable(res.data.data);
    } catch (e) {
      console.error(e);
    }
  ...
  const prevDay = () =>
    setActiveDay((prev) => (prev === 0 ? DAYS.length - 1 : prev - 1));
  const nextDay = () =>
    setActiveDay((prev) => (prev === DAYS.length - 1 ? 0 : prev + 1));
    ...
    <PageTransition>
      <div className="min-h-screen pt-20 pb-12 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-nexura-text-dim">
                Genetic Algorithm with 300 generations × 60 population size
              </p>
            </div>
            {isAdmin && (
            ...
            {showSimPanel && (
              ...
              >
                <GlassCard hover={false} className="mb-6 border-amber-500/20">
                  ...
                  <p className="text-sm text-nexura-text-dim mb-4">
                    Select faculty members to mark as unavailable, then
                    regenerate the timetable.
                  </p>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {faculty.map((f) => {
                      const picked = unavailable.includes(f.employeeId);
                      return (
                        <button
                          key={f.employeeId}
                          ...
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                            picked
                              ...
                              : "bg-white/5 border border-white/10 text-nexura-text-dim hover:border-white/20"
                          }`}
                        >
                          {picked && (
          ...
          {loading && (
            <LoadingSpinner text="Running Genetic Algorithm... Evolving 300 generations" />
          )}
            ...
            >
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-nexura-text">
                        {conflictPct === 0
                          ...
                          : `${timetable.conflictCount} Conflict(s) Found`}
                      </h3>
                      <p className="text-sm text-nexura-text-dim">
                        {timetable.totalSlots} total slots · Fitness score:{" "}
                        {timetable.fitness}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      <div className="text-xs text-nexura-text-muted">
                        Conflict Rate
                      </div>
                      <div
                        className={`text-2xl font-bold font-display ${conflictPct === 0 ? "text-emerald-400" : "text-amber-400"}`}
                      >
                        {conflictPct}%
                      </div>
                    </div>
                    <div className="w-24 h-3 bg-white/5 rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${100 - conflictPct}%` }}
                        transition={{ duration: 1 }}
                        className={`h-full rounded-full ${conflictPct === 0 ? "bg-emerald-500" : "bg-amber-500"}`}
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="md:hidden mb-4">
                <div className="flex items-center justify-between mb-3">
                  <button
                    onClick={prevDay}
                    className="p-2 rounded-lg bg-white/5 hover:bg-white/10 text-nexura-text-dim hover:text-nexura-text transition-colors"
                  >
                    <ChevronLeft className="w-5 h-5" />
                  </button>
                  <span className="text-sm font-semibold text-nexura-cyan">
                    {DAYS[activeDay]}
                  </span>
                  <button
                    onClick={nextDay}
                  ...
                  {DAYS.map((day, i) => (
                    <button
                      ...
                      className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                        activeDay === i
                          ? "bg-nexura-cyan/20 text-nexura-cyan border border-nexura-cyan/30"
                          : "bg-white/5 text-nexura-text-dim hover:bg-white/10"
                      }`}
                    >
                      {day.slice(0, 3)}
                    </button>
                ...
                <div className="mt-3 space-y-2">
                  {TIME_LABELS.map((label, slotIdx) => {
                    const entry = getEntry(DAYS[activeDay], slotIdx + 1);
                    return (
                      <motion.div
                        key={slotIdx}
                      ...
                      >
                        <span className="text-xs text-nexura-text-muted w-20 shrink-0">
                          {label}
                        </span>
                        {entry ? (
                ...
                <div className="min-w-[900px]">
                  <div className="grid grid-cols-[100px_repeat(6,1fr)] gap-1">
                    ...
                    {DAYS.map((day) => (
                      <div
                        key={day}
                        className="p-3 text-center text-sm font-semibold text-nexura-cyan glass rounded-lg"
                      >
                        {day.slice(0, 3)}
                      </div>
                    ))}
                    {TIME_LABELS.map((label, slotIdx) => (
                      <>
                        <div
                          key={`label-${slotIdx}`}
                          className="p-2 text-xs text-nexura-text-muted flex items-center justify-center"
                        >
                          {label}
                        </div>
                        {DAYS.map((day) => {
                          ...
                          return (
                            <motion.div
                              ...
                              transition={{
                                delay: (slotIdx * 6 + DAYS.indexOf(day)) * 0.01,
                              }}
                              className={`p-2 rounded-lg min-h-[70px] border transition-all duration-200 ${
                                entry
                                  ? `${courseColors[entry.courseCode] || "bg-white/5 border-white/10 text-white"} hover:scale-[1.02]`
                                  : "bg-white/2 border-white/5"
                              }`}
                            >
                              {entry && (
              ...
              <GlassCard hover={false} className="mt-6">
                <h4 className="text-sm font-semibold text-nexura-text mb-3">
                  Course Legend
                </h4>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(courseColors).map(([code, cls]) => (
                    <span
                      key={code}
                      className={`px-3 py-1 rounded-lg text-xs border ${cls}`}
                    >
                      {code}
                    </span>
                  ))}
          ...
          {!timetable && !loading && (
            <GlassCard hover={false} className="text-center py-16">
              ...
              <h3 className="text-xl font-semibold text-nexura-text mb-2">
                No Timetable Generated Yet
              </h3>
              <p className="text-nexura-text-dim mb-6">
                {isAdmin
                  ? 'Click "Generate Timetable" to run the AI optimization engine'
                  : "An administrator has not generated the timetable yet."}
              </p>
              {isAdmin && (
                <NeonButton onClick={handleGenerate} icon={Sparkles}>
                  Generate Now
                </NeonButton>
              )}
            </GlassCard>
